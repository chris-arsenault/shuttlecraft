...
impl BrokerConfig {
    pub fn from_env() -> anyhow::Result<Self> {
        ...
        let use_token = std::env::var("SULION_SECRET_BROKER_USE_TOKEN")
            .map_err(|_| anyhow!("SULION_SECRET_BROKER_USE_TOKEN must be set"))?;
        Ok(Self {
            listen,
            db_url,
            master_key_path,
...
impl BrokerState {
    pub async fn from_config(config: &BrokerConfig) -> anyhow::Result<Arc<Self>> {
        ...
        Ok(Arc::new(Self {
            ...
            auth,
            crypto,
            use_token: config.use_token.clone(),
        }))
    }
}
...
pub fn app(state: Arc<BrokerState>) -> Router {
    ...
    let use_routes = Router::new()
        .route("/v1/use", post(use_secret))
        .route_layer(axum::middleware::from_fn_with_state(
            state.clone(),
            require_use_auth,
        ));
...
async fn require_use_auth(
    ...
    let token = bearer_from_request(req.headers(), query.access_token.as_deref());
    if token != Some(state.use_token.as_str()) {
        return unauthorized();
    }
    next.run(req).await
}
...
async fn upsert_secret(
    ...
    let (ciphertext, nonce) = state.crypto.encrypt_env(&body.env)?;
    sqlx::query(
...
async fn use_secret(
    State(state): State<Arc<BrokerState>>,
    Json(body): Json<UseSecretRequest>,
